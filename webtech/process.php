...
if (isset($_POST['confirm'])) {
    // Basic validation example (optional, you can extend)
    if ($password !== $confirm_password) {
        ...
        $password_db_escaped = $conn->real_escape_string($password); // Consider hashing password in real apps!
        $dob_db = $conn->real_escape_string($dob);
        $country_db = $conn->real_escape_string($country);
        $gender_db = $conn->real_escape_string($gender);

        // Insert query
        $sql = "INSERT INTO user (Full_Name, Email, Password, Date_of_Birth, Country, Gender) VALUES 
                ('$fullname_db', '$email_db', '$password_db_escaped', '$dob_db', '$country_db', '$gender_db')";
